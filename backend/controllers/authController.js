...
const googleLogin = async (req, res) => {
    ...
    try {
        ...
        if (user) {
            ...
            // User does not exist
            // Requirement: Mobile is mandatory. We cannot create user without mobile.
            // Return specific error so frontend can show alert
...
const facebookLogin = async (req, res) => {
    ...
    try {
        if (!email) {
            return res.status(400).json({ message: 'Facebook account does not have an email associated.' });
        }
