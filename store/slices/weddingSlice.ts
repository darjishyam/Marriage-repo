...
export const updateWeddingDetails = createAsyncThunk('wedding/updateDetails', async ({ id, data }: { id: string, data: Partial<Wedding> }, { rejectWithValue }) => {
    try {
        const response = await api.put(`/weddings/${id}`, data);
        return response.data;
    } catch (error: any) {
        return rejectWithValue(error.response?.data?.message || 'Failed to update wedding details');
    }
});
...
const weddingSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(updateWeddingDetails.fulfilled, (state, action) => {
                state.weddingData = action.payload;
            })
            // Create Wedding
