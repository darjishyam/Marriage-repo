...
function WeddingDashboard({ weddingData, onSwitch }: { weddingData: { groomName: string; brideName: string; date: Date, totalBudget: number, startStatistics?: { guestCount: number, totalSpent: number }, _id?: string, groomImage?: string, brideImage?: string }, onSwitch: () => void }) {
  const router = useRouter();
  const { guests } = useGuest();
  const { totalAmount } = useExpense();
  const { updateBudget, updateWeddingDetails } = useWedding();
  ...
  // Image View/Update State
  const [showImageModal, setShowImageModal] = useState(false);
  const [selectedImage, setSelectedImage] = useState<{ uri: string; type: 'groom' | 'bride' } | null>(null);

  const handleImagePress = (type: 'groom' | 'bride') => {
    const uri = type === 'groom' ? weddingData.groomImage : weddingData.brideImage;
    // Default images are required, so if uri is undefined we might want to handle it, but for now we assume we can show default in modal if null
    // But actually, passing the URI of the *current* image is better
    // If it's null/undefined, we handle it in render
    setSelectedImage({ uri: uri || "", type });
  ...
  const handleUpdateImage = async () => {
    ...
    if (status !== 'granted') {
      Alert.alert(t("permission_denied"), t("permission_denied_msg") || "Permission to access media library is required to upload images.");
      return;
    }
    ...
    if (!result.canceled && result.assets[0].base64) {
      ...
      const updateData = selectedImage.type === 'groom' ? { groomImage: imageUri } : { brideImage: imageUri };

      await updateWeddingDetails(updateData);
      ...
      <View style={styles.headerArea}>
        ...
        <View style={styles.namesRowHeader}>
          <View style={styles.profileImagesContainer}>
            <TouchableOpacity onPress={() => handleImagePress('bride')}>
              {weddingData.brideImage ? (
                <Image source={{ uri: weddingData.brideImage }} style={[styles.profileImage, styles.brideImage]} />
              ) : (
                <View style={[styles.profileImage, styles.brideImage, { backgroundColor: '#E0E0E0', justifyContent: 'center', alignItems: 'center' }]}>
                  <Ionicons name="person" size={24} color="#FFF" />
                </View>
              )}
            </TouchableOpacity>
            <TouchableOpacity onPress={() => handleImagePress('groom')}>
              {weddingData.groomImage ? (
                <Image source={{ uri: weddingData.groomImage }} style={[styles.profileImage, styles.groomImage]} />
              ) : (
                <View style={[styles.profileImage, styles.groomImage, { backgroundColor: '#DDD', justifyContent: 'center', alignItems: 'center' }]}>
                  <Ionicons name="person" size={24} color="#FFF" />
                </View>
              )}
            </TouchableOpacity>
          ...
          <View style={[styles.modalContent, { alignItems: 'center', padding: 20 }]}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>
                {selectedImage?.type === 'groom' ? t("groom_photo") || "Groom Photo" : t("bride_photo") || "Bride Photo"}
              </Text>
            ...
            {selectedImage?.uri ? (
              <Image
                ...
                style={{ width: 200, height: 200, borderRadius: 100, marginVertical: 20, borderWidth: 2, borderColor: '#eee' }}
              />
            ) : (
              <View style={{ width: 200, height: 200, borderRadius: 100, marginVertical: 20, borderWidth: 2, borderColor: '#eee', backgroundColor: '#F0F0F0', justifyContent: 'center', alignItems: 'center' }}>
            ...
            <TouchableOpacity
              ...
              style={{ backgroundColor: '#000', paddingVertical: 12, paddingHorizontal: 30, borderRadius: 25, flexDirection: 'row', alignItems: 'center' }}
            >
              <Ionicons name="camera" size={20} color="#FFF" style={{ marginRight: 10 }} />
              <Text style={{ color: '#FFF', fontWeight: '600' }}>{t("change_photo") || "Change Photo"}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
